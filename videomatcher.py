...
root = Tkinter.Tk()
rushesFolder = tkFileDialog.askdirectory(parent=root,initialdir="/",title='Please select the root directory that contains all the rushes')
...
for root, dirnames, filenames in os.walk(rushesFolder):
...
def loadAllSmallImageInMemory(thumbnails_folder,add_mirror=True):
    ...
    for i,file in enumerate(listAllSmallImages):
        database[i,:,:]=cv2.imread(file)[:,:,0]
        if i%100==0:
...
def filter(image):
    ...
    return np.abs(scipy.ndimage.convolve(image_float,[[1,-1]]))+np.abs(scipy.ndimage.convolve(image_float,[[1],[-1]]))
...
def retrieveImages(videoFile,thumbnails_folder,threshold=0.1):
    ...
    for i in range(database.shape[0]):
        filtered_database[i,:,:]=filter(database[i,:,:])
    print 'done'
    ...
    while(video.isOpened()):
        ...
        print 'time left %d sec , best matching image for frame %d is %s, with distance %f'%(predicted,idframe,listAllSmallImages[bestframe],distance)
        bestframes[idframe-1]=bestframe
        distancestobest[idframe-1]=distance
        combined_filtred=np.hstack((filtered_small,filtered_database[bestframe]))
...
if True:
    bestframes,distances,distancesMatrix=retrieveImages(videoFile,thumbnails_folder)
...
with open('matched_frame.pkl', 'rb') as f:
    ...
    d=pickle.load(f)
    plt.figure();plt.plot(d['distances'])
    listAllSmallImages,database=loadAllSmallImageInMemory(thumbnails_folder)
    reduced_size=(database.shape[2],database.shape[1])
    videoTable=loadVideoSmall(videoFile,reduced_size)
    plt.figure()
    iplot=plt.imshow(np.zeros((videoTable.shape[1],videoTable.shape[2]*2)),cmap=plt.cm.Greys,vmin=0,vmax=255)
    ...
    if video_writer.isOpened():
        for i in range(combined_video.shape[0]):
            image=np.vstack((np.full((20,new_size[0]),255),cv2.resize(combined_video[i,:,:], (new_size[0],new_size[1]-20)))).astype(np.uint8)
